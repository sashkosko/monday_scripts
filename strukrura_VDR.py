...
for cur_date in date_range:  # Проходимо по всіх датах в заданому діапазоні (від date_start до date_end)
    ...
    for index, row in df.iterrows():
        for labch, accept, accept_percent, objem, objem_percent, apaa in zip(row['lbels_vdr_ukr'], row['akc_vdr_ukr'],
                                                                             row['akc_vdr_ukr_percent'],
                                                                             row['zaiv_vdr_ukr'],
                                                                             row['zaiv_vdr_ukr_percent'],
                                                                             row['apaa_vdr_ukr']):
    ...
    expanded_data = []
    for index, row in df.iterrows():
        for labch, accept, accept_percent, objem, objem_percent, apaa in zip(row['lbels_vdr_ukrp'], row['akc_vdr_ukrp'],
                                                                             row['akc_vdr_ukrp_percent'],
                                                                             row['zaiv_vdr_ukrp'],
                                                                             row['zaiv_vdr_ukrp_percent'],
                                                                             row['apaa_vdr_ukrp']):
            expanded_data.append(
                {'labch': labch, 'Обсяг акцептованої': accept, 'Частка акцептованої': accept_percent,
                 'Обсяг заявленої': objem, 'Частка заявленої': objem_percent, 'apaa': apaa})
    ...
    # В даних за кожну дату містяться дані за дві дати - поточну і за попередній день. Ми залишимо лише дані за попередній день
...
result_df.to_excel(f'{date_end.strftime("%Y_%m_%d")}_VDR_prodazh_kupivlya.xlsx', index=False)
